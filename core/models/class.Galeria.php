...
class Galeria{
	...
	public function getDato(){
		...
		try{
			...
			$sql = $db->query("SELECT * FROM galeria WHERE id=$id;");
	...
	public function getDatos(){
		try{
			...
			$sql = $db->query("SELECT * FROM galeria ORDER BY id DESC LIMIT $offset, $per_page;");
				...
				while($datos = $db->recorrer($sql)){
					...
					$output .= '
						<tr data-rel="'.$datos['id'].'">															
							<td><img src="'.$datos['imagen'].'" style="width: 120px;"></td>
							<td>'.$datos['titulo'].'</td>
					';  
	...
	public function crear(){
	    ...
	    $ejecutar = $db->query("INSERT INTO galeria(titulo) VALUES('$titulo');");

	    if($ejecutar){   	

	    	$sql = $db->query("SELECT id FROM galeria ORDER BY id DESC LIMIT 1;");
	        ...
	        $sql_imagen = $db->query("UPDATE galeria SET imagen='$nombreArchivoFinal' WHERE id=$id;");
		    }
		    //fin subir fotos
	...
	public function editar(){
	    ...
	    //guardando el titulo en la BD
	    $sql = $db->query("UPDATE galeria SET titulo='$titulo' WHERE id=$id;");
	...
	public function getPaginador($reload){
		try{
			...
			$sql = $db->query("SELECT count(*) as conteo FROM galeria;");
	...
	public function getPaginadorWeb($reload){
		try{
			...
			$sql = $db->query("SELECT count(*) as conteo FROM videos;");
	...
	public function getUltimoDato(){
		...
		try{
			...
			$sql = $db->query("SELECT * FROM galeria ORDER BY id DESC LIMIT 1;");
