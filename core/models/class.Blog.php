...
class Blog{
	...
	public function getDato(){
		...
		try{
			...
			$sql = $db->query("SELECT * FROM blog WHERE id=$id;");
	...
	public function getDatos(){
		try{
			...
			$sql = $db->query("SELECT * FROM blog ORDER BY id DESC LIMIT $offset, $per_page;");
	...
	public function crear(){
		try{
			...
			$validar = $db->query("SELECT * FROM blog WHERE url='$url' LIMIT 1;");
	    ...
	    $ejecutar = $db->query("INSERT INTO blog(
			titulo,
			url,
			fecha,
			autor,
	    ...
	    if($ejecutar){
	    	$sql = $db->query("SELECT id FROM blog ORDER BY id DESC LIMIT 1;");
          ...
          $sql_imagen = $db->query("UPDATE blog SET imagen='$nombreArchivoFinal' WHERE id=$id;");
		    }
		    //fin subir fotos
	...
	public function editar(){
	    ...
	    $titulo = str_replace("'", "\'", $_POST['titulo']);
			...
			$validar = $db->query("SELECT * FROM blog WHERE url='$url' AND id!=$id LIMIT 1;");
	...
	public function getDatosLimit($limit){
		try{
			...
			$sql = $db->query("SELECT * FROM blog ORDER BY id DESC LIMIT $limit");
	...
	public function getMasVistos(){
		try{
			...
			$sql = $db->query("SELECT * FROM blog ORDER BY vistas DESC LIMIT 4");
	...
	public function getUltimos($id){
		try{
			$db = new Conexion();

			$sql = $db->query("SELECT * FROM blog WHERE id!=$id ORDER BY id DESC LIMIT 4");
	...
	public function getPaginador($reload){
		try{
			...
			$sql = $db->query("SELECT count(*) as conteo FROM blog;");
	...
	public function getDatoUrl($url){
		...
		try{
			...
			$sql = $db->query("SELECT * FROM blog WHERE url='$url' LIMIT 1;");
