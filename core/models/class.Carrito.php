...
class Carrito{
	...
	public function add($articulo = array()){
		...
		if(!is_array($articulo) || empty($articulo)){
			echo "Error, el articulo no es un array!";	
		}

		//nuestro carro necesita siempre un id producto, cantidad y precio articulo
		if(!$articulo["codigo"] || !$articulo["cantidad"] || !$articulo["precio"]){
			echo "Error, el articulo debe tener un codigo, cantidad y precio!";	
		}

		//nuestro carro necesita siempre un id producto, cantidad y precio articulo
		if(!is_numeric($articulo["cantidad"]) || !is_numeric($articulo["precio"])){
			echo "Error cantidad y precio deben ser números!";	
		}

		//debemos crear un identificador único para cada producto
		$unique_id = md5($articulo["codigo"]);
	...
	//método que retorna el número de artículos del carrito, teniendo en cuenta su cantidad
	...
	public function update_cantidad_articulos(){
		//asignamos el carrito a una variable
		$carrito = $this->carrito;
		//debemos eliminar del carrito el número de artículos
		//y el precio total para poder mostrar bien los artículos
	...
	public function update_cantidad($unique_id, $cantidad){
		//modificamos la cantidad del producto por unique_id
